...
class MappingAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
             return view('pages-admin.setting.mapping-admin.data')
                ->with('user',$user)
                ->with('hal',$hal)
                ->render();
        }
        ...
        return view('pages-admin.setting.mapping-admin.index')
                ->with('hal',$hal)
                ->with('user',$user);
    }
    ...
    public function show($id=-1)
    {
        ...
        return view('pages-admin.setting.mapping-admin.form')
            ->with('det',$det)
            ->with('user',$user)
            ->with('id',$id);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect('mapping-admin')->with('status', 'Tambah Data Admin Baru Berhasil');
    }
}
