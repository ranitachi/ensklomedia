                    ...
                    @foreach ($psrt[$v->id] as $item)
                        <div class="row" style="margin-bottom:10px;">
                            @if ($item->flag==0)
                                ...
                                <div class="col-md-9" style="color:red" data-toggle="tooltip" title="Peserta Belum Aktif"><b>{{$item->name=='n/a'||$item->name=='' ? $item->user->email : $item->name}}</b></div>
                            @else
                                <div class="col-md-3"><i class="fa fa-close" style="color:purple;cursor:pointer" data-toggle="tooltip" title="Non Aktifkan Peserta" onclick="aktifkanpeserta('{{$item->idpf}}','0')"></i>
                                 <i class="fa fa-trash" style="color:red;cursor:pointer" data-toggle="tooltip" title="Hapus Peserta" onclick="hapuspeserta('{{$item->idpf}}')"></i>
                                </div>    
                                <div class="col-md-9" style="color:green"><b><span class="label label-success" style="font-size:12px;">{{$item->name=='n/a'||$item->name=='' ? $item->user->email : $item->name}}</span></b></div>
                            @endif
                        </div>
                    @endforeach
                @endif
