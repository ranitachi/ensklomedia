...
@section('title')
    <title>Form PIC Fasilitasi - Ensiklomedia</title>
             ...
             <div id="category" style="margin-top:20px;">
            	<div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="row">
                            <div class="col-md-6" >
                                <h1 class="new-video-title"><i class="fa fa-users"></i> Form {{$id==-1 ? 'Tambah' : 'Edit'}} PIC Fasilitasi</h1>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="pull-right">
                                    <a href="{{url('mapping-admin')}}" class="btn btn-danger">
                                        <i class="fa fa-angle-double-left"></i> Kembali
                                    </a>
                                </div>
                            </div>
                        </div>
                        <form id="add-admin" action="{{$id==-1 ? URL::to('mapping-fasilitasi') : URL::to('mapping-fasilitasi/'.$id) }}" method="POST" enctype="multipart/form-data">
                            @if ($id!=-1)
								{{ method_field('PATCH') }}
                            @endif
                            ...
                            <div class="row" style="margin-top:10px;" id="">
                                <div class="col-md-12" >
                                        <select multiple="multiple" size="10" id="duallistbox_demo1" name="nama_admin[]">
                                            @foreach ($user as $k => $v)
                                                @if ($v->authorization_level==5)
                                        ...
                                        <br>
                                        <button type="submit" class="btn btn-default btn-block">Simpan Mapping PIC Fasilitasi</button>
...
@section('footscript')
    ...
    <script>
        ...
        var demo1 = $('select[name="nama_admin[]"]').bootstrapDualListbox({
        ...
        $("#add-admin").submit(function() {
            ...
            if(sv=='')
            {
                alert('Silahkan Pilih Pengguna yang akan dijadikan Admin')
                return false;
            }
            else
            {
